...
- 脚本每天执行一次：北京时间早上 8 点（GitHub Actions cron 使用 UTC，对应 `0 0 * * *`）
- 你也可以随时手动触发签到

## 调试日志（可选）

如需在 Actions 或本地查看更多“安全”调试信息（状态码/响应头/跳转 location），可开启：

- `DEBUG_HTTP=1`：打印 HTTP 调试信息（不包含响应正文）

如需进一步查看接口返回内容（用于排查签到/余额等逻辑），可额外开启：

- `DEBUG_HTTP_BODY=1`：打印响应正文片段（可能包含敏感信息，谨慎开启）
- `DEBUG_HTTP_MAX_CHARS=800`：正文片段最大输出长度（默认 800）
